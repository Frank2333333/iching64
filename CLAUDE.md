...
- **潜能雷达图（六维，混合打分，结果页 PersonalityCard 下方）**：六轴 = 事业魄力/财富积累/人际魅力/创造思维/抗压稳定/行动执行。**混合打分**——`life-report-ai.ts` 的 `computeRadarBaseScores(baziChart, mbti)` 本地确定性算基础分（每轴 base 50 + 八字相关十神计数加减 + 日主强弱 + MBTI 字母匹配 ±10，clamp 5..95）；`getRadarScores()` 让 AI 结合紫微盘对应宫位（官禄/财帛/夫妻+桃花/命宫/疾厄+田宅）在**基础分 ±15 内**微调（硬性 clamp 防漂移）+ 每轴一句紫微点评，JSON 模式温度 0.2。后端 `/api/life-report/radar`（auth + chat 额度）。前端 `src/components/life-report/RadarChart.tsx`（纯 SVG 六维雷达，深色金色，forwardRef 可分享）+ `RadarCard.tsx`（雷达+各轴点评+分享，复用 svg-to-png）。按 `iching_radar_<生辰+mbti>` 缓存到 localStorage，同盘不重复消耗额度。框定为"潜能能量分布"非命数成绩单。
