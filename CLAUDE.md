...
- **潜能雷达图（六维，混合打分，结果页 PersonalityCard 下方）**：六轴 = 事业魄力/财富积累/人际魅力/创造思维/抗压稳定/行动执行。**混合打分**——`life-report-ai.ts` 的 `computeRadarBaseScores(baziChart, mbti)` 本地确定性算基础分：基线 55 + **格局主导轴 +12**（`patternAxis` 把正官/七杀格→事业、正偏财格→财富、食伤格→创造、正偏印格→抗压、其他格→执行）+ **用神/喜神/忌神得力**（把各轴十神类别换算五行对照用神 +8/喜神 +4/忌神 -8）+ 藏干十神计数中性微调 + 日主强弱 + MBTI 字母 ±10，clamp 12..92。注意 `pillar.shiShen` 是**藏干十神**（bazi-calculator 第147行），月干/格局十神不在其中，故用 `pattern` 字段而非计数来抓主导十神。`getRadarScores()` 让 AI 结合紫微盘对应宫位（官禄/财帛/夫妻+桃花/命宫/疾厄+田宅）在**基础分 ±15 内**微调（硬性 clamp 防漂移）+ 每轴一句紫微点评，JSON 模式温度 0.2。后端 `/api/life-report/radar`（auth + chat 额度）。前端 `RadarChart.tsx`（纯 SVG 六维雷达，深色金色，forwardRef 可分享）+ `RadarCard.tsx`。按 `iching_radar_v2_<生辰+mbti>` 缓存（v2 失效旧公式缓存）。框定为"潜能能量分布"非命数成绩单。
