...
function loadSkillContent() {
  ...
  try {
    ...
    if (fs.existsSync(skillPath)) {
      ...
      console.log('[BaziAI] 已加载 SKILL.md');
    } else {
  ...
  try {
    if (fs.existsSync(REFS_DIR)) {
      ...
      console.log(`[BaziAI] 已加载 ${files.length} 个 reference 文件`);
    }
  } catch (err) {
...
/**
 * 获取 AI 八字排盘解读
 * @param {Object} input - 用户输入的出生信息
 * @returns {Promise<string>} AI 解读内容
 */
...
async function chatWithBazi(data) {
  ...
  if (baziInput) {
    ...
    if (pillars) {
      ...
      contextParts.push(`命主信息：${gender === 'male' ? '男' : '女'}，${year}年${month}月${day}日 ${hour}:${minute.toString().padStart(2, '0')}，${birthplace || '地点未提供'}`);
    }
  }
