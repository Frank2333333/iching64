/**
 * OpenAI 解卦服务 — Workers 版
 * 从 server/divination-ai.cjs 移植，纯逻辑+OpenAI 调用，无 fs 操作
 */
import OpenAI from 'openai';

// ==================== 八卦基础信息表 ====================

const baGuaInfo: Record<string, { wuxing: string; nature: string; direction: string; number: number; symbol: string }> = {
...
function getGuaWuxing(guaName: string): string {
  return baGuaInfo[guaName]?.wuxing || '未知';
}

function getWuxingRelation(tiWuxing: string, yongWuxing: string): string {
...
interface SceneOption { id: string }

function getSceneFollowUpGuidance(selectedScene: SceneOption | null | undefined, questionContent: string | undefined) {
  const fallback = {
    sceneName: '通用问事',
    target: '当前所问之事',
    ...
    opener: '对这件事来说',
    originalQuestion: questionContent ? `用户原始问题是「${questionContent}」` : '用户未补充更具体的问题',
  };

  if (!selectedScene?.id) return fallback;

  const sceneRules: Record<string, typeof fallback> = {
    career: {
      sceneName: '事业前程', target: '工作、项目、升迁或职业发展',
      ...
      opener: '对事业来说', originalQuestion: '',
    },
    relationship: {
      sceneName: '感情姻缘', target: '这段感情、这段关系或双方互动',
      ...
      opener: '对感情来说', originalQuestion: '',
    },
    health: {
      sceneName: '健康疾病', target: '身体状态、恢复进展或调养效果',
      ...
      opener: '对健康来说', originalQuestion: '',
    },
    wealth: {
      sceneName: '财运投资', target: '求财结果、投资收益或资金安排',
      ...
      switchRule: '只有当用户明确改问其他领域时，才允许切换。',
      opener: '对财运来说', originalQuestion: '',
    },
    study: {
      sceneName: '学业考试', target: '学习状态、考试发挥或升学结果',
      ...
      switchRule: '只有当用户明确改问其他领域时，才允许切换。',
      opener: '对学业来说', originalQuestion: '',
    },
    travel: {
      sceneName: '出行迁移', target: '出行计划、迁移安排或途中变化',
      ...
      switchRule: '只有当用户明确改问其他领域时，才允许切换。',
      opener: '对出行来说', originalQuestion: '',
    },
    legal: {
      sceneName: '官司诉讼', target: '诉讼、纠纷或调解结果',
      ...
      switchRule: '只有当用户明确改问其他领域时，才允许切换。',
      opener: '对官司这件事来说', originalQuestion: '',
    },
    lost: {
      sceneName: '寻物失物', target: '失物找回结果、寻找方向或时间线索',
      ...
      switchRule: '只有当用户明确改问其他领域时，才允许切换。',
      opener: '对寻物这件事来说', originalQuestion: '',
    },
  };

  const rule = sceneRules[selectedScene.id];
  if (!rule) return fallback;
  return { ...rule, originalQuestion: questionContent ? `用户原始问题是「${questionContent}」` : '用户未补充更具体的问题' };
}

// ==================== 追问意图分类 ====================

function classifyFollowUpIntent(message: string) {
  const normalized = (message || '').replace(/\s+/g, '');

  if (!normalized) {
    return { type: 'judgment', label: '判断类', goal: '回答当前场景对象整体偏好、偏坏还是中性。', answerPattern: '第一句直接下判断；第二句解释原因；第三句补一句建议。', examples: '如"好不好""是好还是坏""顺不顺"' };
  }

  if (/(什么时候|何时|几时|多久|多快|哪天|几天|几个月|啥时候|何时有消息|什么时候行动|什么时候出发|时间|时机)/.test(normalized)) {
    return { type: 'timing', label: '时间类', goal: '回答当前场景对象何时行动更合适、何时见结果、何时容易有消息。', answerPattern: '第一句给时间判断；第二句说明早晚快慢；第三句给行动时点建议。', examples: '如"什么时候""多久有结果""什么时候行动更好"' };
  }

  if (/(注意什么|需要注意|特别注意|该注意|小心什么|提防|避免|怎么办|怎么做|如何做|要怎么|怎么准备|如何准备|怎么应对|建议|注意事项|要不要|该不该)/.test(normalized)) {
    return { type: 'advice', label: '建议类', goal: '回答当前场景对象最该注意什么、该做什么、该避免什么。', answerPattern: '第一句点出最重要的注意点；第二句说明风险或关键点；第三句给具体做法。', examples: '如"要注意什么""该怎么办""要不要继续"' };
  }

  if (/(结果|结局|最终|后来|之后|会怎样|能成吗|成不成|能不能成|会不会成功|能否成功|能否成行|最后会怎样|有没有结果|会不会好起来)/.test(normalized)) {
    return { type: 'outcome', label: '结果类', goal: '回答当前场景对象最终走向、能否达成、会是什么结果。', answerPattern: '第一句说最终倾向；第二句解释中间变化；第三句给应对建议。', examples: '如"最后会怎样""能不能成""结果如何"' };
  }
  ...
  return { type: 'advice', label: '建议类', goal: '优先回答当前场景中最有行动价值的建议和注意点。', answerPattern: '第一句先回应用户最关心的问题；第二句补充关键原因；第三句给建议。', examples: '默认用于没有明显类型、但需要具体回应的追问' };
}

function buildUserFollowUpTrack(history: Array<{ role: string; content: string }>) {
  const recentUserMessages = (history || [])
    .filter((message) => message.role === 'user' && message.content)
    .slice(-3);

  if (recentUserMessages.length === 0) return '（暂无更早的追问记录）';
...
interface DongYao { position: number; name: string; yinYang: string; text: string; xiangZhuan: string }
interface GuaData { id: number; chineseName: string; name: string; shangGua: string; xiaGua: string; meaning: string; guaci: string }
interface HuGua { shangGuaName: string; xiaGuaName: string }
interface BianGua { shangGuaName: string; xiaGuaName: string }
interface WuxingDetail { judgment: string; description: string }
...
interface DivinationData {
  ...
  huGua: HuGua | null;
  bianGua: BianGua | null;
  selectedScene: SceneOption | null;
  questionContent: string | undefined;
  divinationTime: string | undefined;
}

function buildPrompt(data: DivinationData): string {
  const { gua, dongYao, tiGuaName, yongGuaName, wuxingDetail, huGua, bianGua, selectedScene, questionContent, divinationTime } = data;

  const sceneMap: Record<string, { name: string; tips: string }> = {
  ...
  const seasonMap: Record<string, string> = { '木': '春', '火': '夏', '金': '秋', '水': '冬', '土': '四季' };
...
const SYSTEM_PROMPT_DIVINATION = `你是一位融汇《梅花易数》《易经》与传统命理典籍的易学大师。解卦时遵循以下原则：
...
const SYSTEM_PROMPT_CHAT = `你是一位融汇《梅花易数》《易经》与传统命理典籍的易学顾问。追问回答时遵循：
1. 铁口直断：先给判断，再解释
2. 证据链：每个判断需有卦象依据和现实落点
3. 安全边界：禁用"必然、一定、注定"，健康只说体质倾向，财富不给投资指令，感情不做道德审判
4. 严格围绕用户当前问事场景作答，不得擅自切换到其他场景`;

export async function getAIDivination(divinationData: DivinationData, env: Env): Promise<string> {
  ...
  try {
    ...
    console.log(`[DivinationAI] 调用 OpenAI 解卦，模型: ${model}`);
    ...
    if (!result) throw new Error('OpenAI 返回结果为空');

    console.log(`[DivinationAI] 解卦成功，tokens: ${response.usage?.total_tokens || 'unknown'}`);
    return result;
  } catch (error: unknown) {
    const err = error as { code?: string; message?: string };
    if (err.code === 'insufficient_quota') throw new Error('OpenAI API 额度不足，请联系管理员');
    if (err.code === 'invalid_api_key') throw new Error('OpenAI API Key 无效，请检查配置');
    if (err.code === 'rate_limit_exceeded') throw new Error('请求过于频繁，请稍后再试');
    if (err.message?.includes('timeout')) throw new Error('请求超时，请稍后重试');
    throw new Error(`解卦服务暂时不可用: ${err.message}`);
  }
}

export async function chatWithAI(data: ChatData, env: Env): Promise<string> {
  if (!env.OPENAI_API_KEY || env.OPENAI_API_KEY === 'your-openai-api-key-here') {
    throw new Error('OpenAI API Key 未配置');
  }
  ...
  try {
    ...
    console.log(`[DivinationAI] 调用 OpenAI 对话，模型: ${model}`);
    ...
    console.log(`[DivinationAI] 对话成功，tokens: ${response.usage?.total_tokens || 'unknown'}`);
    return result;
  } catch (error: unknown) {
    const err = error as { code?: string; message?: string };
    if (err.code === 'insufficient_quota') throw new Error('OpenAI API 额度不足，请联系管理员');
    ...
    if (err.code === 'rate_limit_exceeded') throw new Error('请求过于频繁，请稍后再试');
    if (err.message?.includes('timeout')) throw new Error('请求超时，请稍后重试');
    throw new Error(`对话服务暂时不可用: ${err.message}`);
  }
}
