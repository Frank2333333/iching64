...
export default function ZiweiChatInput({
  input,
  loading,
  disabled,
  onInputChange,
    ...
    <div className="flex-none border-t border-amber-200/80 dark:border-amber-900/30 bg-white/90 dark:bg-neutral-900/90 backdrop-blur-xl px-4 py-3 sm:px-6">
      <div className="mx-auto max-w-5xl">
        <div className="flex gap-2">
          ...
          />
          <button onClick={onSend} disabled={!input.trim() || disabled}
            className="px-4 py-2 bg-amber-600 hover:bg-amber-700 disabled:bg-gray-400 disabled:cursor-not-allowed
            ...
            aria-label="发送">
            {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <span className="text-lg">➤</span>}
          </button>
        </div>
